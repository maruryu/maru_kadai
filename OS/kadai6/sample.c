...
void checkButton(VP_INT exinf)
{
	...
	while(1) {
		...
		if ((stat=(oldbutton^button))) {
			...
			}else if(oldbutton>button) { /*押された*/
				switch (stat){
				case BTN0:
					sta_tsk(tskID_B,BTN1R);
					break;
				case BTN1:
					sta_tsk(tskID_B,BTN2R);
					break;
				case BTN2:
					sta_tsk(tskID_B,BTN3R);
					break;
				case BTN3:
					sta_tsk(tskID_B,BTN4R);
					break;
				default:
					break;
				}
